...
// Add this function to sell multiple products
export const sellMultipleProducts = async (products: Product[]):
    Promise<Product[]> => {
    try {
        const response = await axios.post(`${API_URL}/products/sell-multiple`, products);
        return response.data;  // Returns updated product list if successful
    } catch (error) {
